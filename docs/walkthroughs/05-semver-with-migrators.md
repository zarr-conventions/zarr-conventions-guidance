...
Semver with migrators is the most robust contract on the menu and the most expensive to operate. It is the only contract that handles `semantic-only` and `add-with-semantic-shift` cleanly across the version transition: the migrator normalizes old data into new shape before processing, so the v2 reader's single code path serves every supported release. The cost is that the author must write the migrators, the spec must ship them, and the reader must implement them. If any link breaks, the contract degrades to the same failure modes as [Integer-major with structural pass-through](/walkthroughs/04-integer-major-pass-through) or worse.
