...
This loose-semver pattern is essentially [EffVer (effort versioning)](https://jacobtomlinson.dev/effver/) by another name: version numbers reflect the upgrade effort required (none / some / significant) rather than strict break-compatibility. STAC's minor bumps are honest EffVer-minors, whether or not the maintainers call them that. A convention shipping under Contract 5 may find that explicitly adopting EffVer naming is more honest than calling its release pattern "semver in practice."
